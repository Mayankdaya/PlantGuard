...
if ($Help) {
    ...
    Write-Host ""
    Write-Host "Usage: .\deploy_render_cli.ps1 [-Action <action>] [-ServiceName <name>]" -ForegroundColor Yellow
    Write-Host ""
    ...
    Write-Host "  .\deploy_render_cli.ps1" -ForegroundColor Gray
    Write-Host "  .\deploy_render_cli.ps1 -Action login" -ForegroundColor Gray
    Write-Host "  .\deploy_render_cli.ps1 -ServiceName my-plant-app" -ForegroundColor Gray
    exit 0
}
    ...
    "deploy" {
        ...
        if ($confirmDeploy -eq 'y') {
            ...
            try {
                ...
                # Note: Render CLI doesn't have a direct "create service" command
                # It works by connecting your GitHub repository through the web interface
                # The render.yaml file will be automatically detected and used for configuration
                ...
                Write-Host "Run: .\deploy_render_cli.ps1 -Action login" -ForegroundColor Yellow
                exit 1
            }
        } else {
    ...
    "status" {
        ...
        try {
            ...
            if ($plantGuardServices) {
                ...
                Write-Host "ℹ️  No PlantGuard services found. Deploy first using: .\deploy_render_cli.ps1" -ForegroundColor Yellow
            }
        } catch {
